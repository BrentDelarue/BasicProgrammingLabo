...
decimaalgetal = 0.0
while element != ",":
    if getal[waarde1:waarde1] != ",":
        element = getal[waarde1:waarde1]
        waarde1 += 1
    else:
        element = getal[waarde1:waarde1]
geheel = getal[:(waarde1-1)]
decimaal = getal[(waarde1+1):]
while waarde2 != waarde1:
    waarde3 = waarde1-1
    geh = int(getal[waarde2:waarde2])
    geh = geh*(2**waarde3)
    geheelgetal += geh
    waarde3 -= 1
    waarde2 += 1
...
while waarde4 != maximaal+1:
    dec = float(getal[waarde4:waarde4])
    dec = dec*(2**(-(waarde4+1)))
    decimaalgetal += dec
    waarde4 += 1
